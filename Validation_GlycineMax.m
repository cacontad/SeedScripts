...
% 28DAF biomass rxn
model28 = model2;
model28 = changeObjective(model28,'BIOMASS-RXN_C08_28DAF',1);
s = optimizeCbModel(model28);
fprintf('\n28 DAF WT growth rate no nutrients: %.4f\n', s.f)
model28 = changeObjective(model28,'BIOMASS-RXN_C08_28DAF_ISOFLAVONES',1);
s = optimizeCbModel(model28);
fprintf('\n28 DAF_ISO WT growth rate no nutrients: %.4f\n', s.f)

% 40DAF biomass rxn
model40 = model2;
model40 = changeObjective(model40,'BIOMASS-RXN_C08_40DAF',1);
s = optimizeCbModel(model40);
fprintf('\n40 DAF WT growth rate no nutrients: %.4f\n', s.f)
model40 = changeObjective(model40,'BIOMASS-RXN_C08_40DAF_ISOFLAVONES',1);
s = optimizeCbModel(model40);
fprintf('\n40 DAF_ISO WT growth rate no nutrients: %.4f\n', s.f)

% 60DAF biomass rxn
model60 = model2;
model60 = changeObjective(model60,'BIOMASS-RXN_C08_60DAF',1);
s = optimizeCbModel(model60);
fprintf('\n60 DAF WT growth rate no nutrients: %.4f\n', s.f)
model60 = changeObjective(model60,'BIOMASS-RXN_C08_60DAF_ISOFLAVONES',1);
s = optimizeCbModel(model60);
fprintf('\n60 DAF_ISO WT growth rate no nutrients: %.4f\n', s.f)

% MATURE biomass rxn
modelM = model2;
modelM = changeObjective(modelM,'BIOMASS-RXN_C08_MATURE',1);
s = optimizeCbModel(modelM);
fprintf('\nMATURE WT growth rate no nutrients: %.4f\n', s.f)
modelM = changeObjective(modelM,'BIOMASS-RXN_C08_MATURE_ISOFLAVONES',1);
s = optimizeCbModel(modelM);
